...
- **Resume song + back gesture:** `restoreOpen()` (launch/refresh resuming the last song) shows the song but **defers** its `history.pushState` until the first user interaction — a push during load is flagged skippable by Android Chrome's back/forward intervention, so the first Back would exit the app. `closeSong()` + in-app Back/brand disarm the un-pushed buffer (`pendingSongEntry`). Tapping a song from the list pushes immediately (already a user gesture).
